...
The propositions below apply to two-player zero-sum games. Section~\ref{sec:boundaries} presents empirical evidence that cooperative settings produce bounded degradation rather than collapse; formal extension to general-sum games is future work.
...
\textbf{Non-zero-sum domains.} In the cooperative Coordination game, forcing P0 to a single action (CAC $= 0$) degrades performance ($p = 0.001$) but does not produce convergence to the DEA. In the Negotiation game, P1 shifts toward rejection because it can condition on P0's inability to adapt, but does not converge to unconditional rejection. Cooperative dynamics produce bounded degradation, not collapse---the threshold interacts with interaction structure.
